 ...
 *
 * @RestResource(
 *   id = "nicol_messagetypes_anon",
 *   label = @Translation("Nicol messagetypes for berichtenbox"),
 *   uri_paths = {
 *     "canonical" = "/api/v1/messagetypes_anon"
 *   }
 * )
 */
class NicolMessageTypesAnon extends ResourceBase {
  ...
  public function get($data, $request) {
    ...
    $items = $container->get('entity.manager')->getStorage('taxonomy_term')->loadTree('berichttypen', 0, 1, TRUE);
    ...
    if (!empty($items)) {
      ...
      foreach ($items as $parent) {
        ...
        // Filter only selected service.
        if ((isset($filter['service']) && $filter['service'] !== $parent->id()) || (!in_array($valid_app, $apps))) {
          continue;
        }
        ...
        foreach ($terms as $child) {
          $linked_apps = $child->field_linked_to_app->getValue();
          $apps = [];
          foreach ($linked_apps as $app) {
            $apps[] = $app['value'];
          }
          if (in_array($valid_app, $apps)) {
