...
/**
 * Provides announcements for anonymous users, only containt announcements with
 *  access for anonymous users
 *
 * @RestResource(
 *   id = "nicol_announcements_anonymous",
 *   label = @Translation("Nicol announcements for anonymous users"),
 *   uri_paths = {
 *     "canonical" = "/api/v1/announcements_anon",
 *     "https://www.drupal.org/link-relations/create" = "/api/v1/announcements_anon"
 *   }
 * )
 */
class NicolAnnouncementsAnon extends ResourceBase {
  ...
  public function post(array $data) {
    ...
    $result = $this->getAnnouncements($status, $valid_message_types);
  ...
  private function getAnnouncements($status = ['open', 'closed'], $filter = []) {
    ...
    $nids = $query->sort('created', 'DESC')->execute();
